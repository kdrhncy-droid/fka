...
export function movePlayer(
  ...
  // İstasyon collision'ları
  const STATION_HALF_W = 35; // İstasyon genişliği/2
  const STATION_HALF_H = 35; // İstasyon yüksekliği/2
  
  function checkStationCollision(stationX: number, stationY: number) {
    const left = stationX - STATION_HALF_W, right = stationX + STATION_HALF_W;
    const top = stationY - STATION_HALF_H, bottom = stationY + STATION_HALF_H;
    if (nx + PR > left && nx - PR < right && ny + PR > top && ny - PR < bottom) {
      const oL = (nx + PR) - left, oR = right - (nx - PR);
      const oT = (ny + PR) - top,  oB = bottom - (ny - PR);
      const min = Math.min(oL, oR, oT, oB);
  ...
  // Sabit istasyonlar
  const trashPos = gs.stationLayout?.['trash'] ?? { x: 920, y: 285 }; // TRASH_STATION import etmek yerine
  checkStationCollision(trashPos.x, trashPos.y);

  const trayPos = gs.stationLayout?.['tray'] ?? { x: 300, y: 285 }; // TRAY_STATION
  checkStationCollision(trayPos.x, trayPos.y);

  const dirtyTrayPos = gs.stationLayout?.['dirty_tray'] ?? { x: 860, y: 90 }; // DIRTY_TRAY_POS
  checkStationCollision(dirtyTrayPos.x, dirtyTrayPos.y);

  // Baharat rafı (gün 3+)
  if (gs.day >= 3) {
    const spicePos = gs.stationLayout?.['spice_rack'] ?? { x: 500, y: 90 }; // SPICE_RACK_POS
    checkStationCollision(spicePos.x, spicePos.y);
  }

  // Tabak yığını - daha büyük collision
  const platePos = gs.stationLayout?.['plate_stack'] ?? { x: 400, y: 90 };
  const PLATE_HALF_W = 25, PLATE_HALF_H = 25;
  const plateLeft = platePos.x - PLATE_HALF_W, plateRight = platePos.x + PLATE_HALF_W;
  const plateTop = platePos.y - PLATE_HALF_H, plateBottom = platePos.y + PLATE_HALF_H;
  if (nx + PR > plateLeft && nx - PR < plateRight && ny + PR > plateTop && ny - PR < plateBottom) {
    const oL = (nx + PR) - plateLeft, oR = plateRight - (nx - PR);
    const oT = (ny + PR) - plateTop,  oB = plateBottom - (ny - PR);
    const min = Math.min(oL, oR, oT, oB);
    if (min === oL) nx = plateLeft - PR;
    else if (min === oR) nx = plateRight + PR;
    else if (min === oT) ny = plateTop - PR;
    else ny = plateBottom + PR;
  }

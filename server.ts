...
io.on("connection", (socket) => {
  ...
  socket.on("join", ({ room, roomId: clientRoomId, name, color, hat, charType, hairColor, clothingColor, faceShape, nameLabelColor, title, labelEffect, mapId }) => {
    ...
    gs.players[socket.id] = {
      ...
      hairColor, clothingColor, faceShape, nameLabelColor, title, labelEffect,
  ...
  socket.on("updateAppearance", (data: { hairColor?: string, clothingColor?: string, faceShape?: number, color?: string, hat?: string, nameLabelColor?: string }) => {
    if (!roomId || !RoomManager.getRoomState(roomId)) return;
    const gs = RoomManager.getRoomState(roomId)!;
    if (gs.players[socket.id]) {
